// src/Component/Sidebar/DoctorSidebar.jsx
...
const DoctorSidebar = memo(function DoctorSidebar({ activeView, setActiveView }) {
  ...
  // Restore last scroll
  useEffect(() => {
    const savedScroll = localStorage.getItem("doctorSidebarScroll");
  ...
  const handleScroll = () => {
    if (sidebarRef.current) {
      localStorage.setItem("doctorSidebarScroll", sidebarRef.current.scrollTop);
    }
  };
      ...
      >
        ...
        {(open || isDrawer) && (
          <Typography variant="body2" sx={{ ml: 1, mb: 0.7, color: "rgba(255,255,255,0.7)", fontSize: { xs: "0.9rem", sm: "0.8rem" } }}>
            {t("quickCreate", language)}
          </Typography>
        )}
        <Tooltip title={t("createCenter", language)} placement={isRTL ? "left" : "right"} disableHoverListener={open || isDrawer}>
          <ListItemButton
            onClick={() => handleNavClick("create-center")}
            selected={activeView === "create-center"}
            sx={{ "&.Mui-selected": { backgroundColor: "rgba(255,255,255,0.2)" } }}
          >
            <ListItemIcon>
              <NoteAddIcon sx={{ color: "#FFDE59", fontSize: { xs: 28, sm: 24 } }} />
            </ListItemIcon>
            {(open || isDrawer) && <ListItemText primary={t("createCenter", language)} />}
          </ListItemButton>
        </Tooltip>
        <Divider sx={{ my: 1, borderColor: "rgba(255,255,255,0.2)" }} />
  ...
  return (
    <>
      ...
      <LogoutDialog
        ...
        onConfirm={() => {
          clearAuthData();
          localStorage.removeItem("doctorSidebarScroll");
