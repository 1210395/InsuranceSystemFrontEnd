...
const PendingRequests = () => {
  ...
  const [filterRole, setFilterRole] = useState("ALL"); // Filter by requested role
  ...
  useEffect(() => {
    const fetchAllRequests = async () => {
      try {
        ...
        // داخل fetchAllRequests بعد الجلب
const filtered = res.data.filter(
  ...
  const getCardColor = useCallback((status) => {
    switch (status) {
      case "APPROVED":
  ...
  useEffect(() => {
    if (filterRole === "ALL") {
      ...
      const filtered = allClients.filter(
        (client) => client.requestedRole?.toUpperCase() === filterRole.toUpperCase()
      );
      setClients(filtered);
    }
  ...
  const handleRejectConfirm = async () => {
    ...
    try {
      await api.patch(API_ENDPOINTS.CLIENTS.REJECT(selectedClient.id), { reason: rejectReason });
...
const _fetchClientFamily = async (client) => {
  setSelectedClient(client);
...
<Grid item xs={12} md={6}>
  <Paper sx={{ p: 2, borderRadius: 2 }}>
    ...
    <Stack spacing={1}>
      ...
      {/* إظهار التخصص الطبي وموقع العيادة للدكتور */}
      ...
      {/* إظهار الحقول الخاصة بدكتور الأشعة */}
