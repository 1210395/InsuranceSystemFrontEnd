...
const LabProfileComponent = ({ userInfo, setUser, refresh }) => {
  ...
  const [formData, setFormData] = useState({
    employeeId: userInfo?.employeeId || userInfo?.labCode || "",
  ...
  React.useEffect(() => {
    if (userInfo) {
      ...
      setFormData((prev) => ({
        ...prev,
        employeeId: userInfo.employeeId || userInfo.labCode || prev.employeeId,
  ...
  const handleSave = async () => {
    try {
      ...
      const res = await api.patch("/api/labs/me/update", form, {
        headers: { "Content-Type": "multipart/form-data" },
      });

      // ✅ تحديث البيانات
      setUser(res.data);
      localStorage.setItem("labUserInfo", JSON.stringify(res.data));
      if (refresh) refresh();
      ...
      if (newImagePath) {
        ...
        localStorage.setItem("labProfileImage", newUrl);
      }
      ...
      console.error("Error updating lab profile:", err.response || err);
          ...
          <Grid xs={12} md={8}>
            <Grid container spacing={3}>
              ...
              <Grid xs={12} md={6}>
                ...
                <Chip
                  label={userInfo?.roles?.[0] || "LAB_TECH"}
                  sx={{
                    mr: 1,
                    background: "#556B2F",
                    color: "#fff",
...
LabProfileComponent.propTypes = {
  userInfo: PropTypes.shape({
    employeeId: PropTypes.string,
    labCode: PropTypes.string,
...
LabProfileComponent.defaultProps = {
  userInfo: null,
  refresh: null,
};

const LabProfile = memo(LabProfileComponent);
