...
function AddSearchProfilePharmacist({ refresh }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!form.name || form.name.trim() === "") {
        errors.push(t("pharmacyNameRequired", language));
      }
      ...
      if (response.status === 200 || response.status === 201) {
        setSnackbar({
          open: true,
          message: t("pharmacyProfileCreatedSuccessfully", language),
          severity: "success",
        });
      ...
      console.error("Error creating pharmacist search profile:", err);

      let errorMessage = t("failedToCreateProfile", language);

      if (err.response?.status === 400) {
        errorMessage = t("invalidDataPleaseCheck", language);
      } else if (err.response?.status === 403) {
        errorMessage = t("noPermissionToCreateProfiles", language);
      } else if (err.response?.status === 409) {
        errorMessage = t("profileWithNameExists", language);
  ...
  const handleLocationSelect = (lat, lng) => {
    ...
    setSnackbar({
      open: true,
      message: t("locationSelectedSuccessfully", language),
      severity: "success",
    });
  };
            ...
            <Stack direction="row" alignItems="center" spacing={2} mb={2}>
              ...
              <Box>
                <Typography variant="h4" fontWeight="700" sx={{ mb: 0.5 }}>
                  {t("addPharmacistProfile", language)}
                </Typography>
                ...
                <Stack spacing={3}>
                      ...
                      {form.locationLat && form.locationLng && (
                        <Box sx={{ p: 2, backgroundColor: "#E8F5E9", borderRadius: 2, border: "1px solid #4CAF50" }}>
                          ...
                          <Typography variant="caption" sx={{ color: "#558B2F", display: "block" }}>
                            {t("latitude", language)}: {form.locationLat.toFixed(6)} | {t("longitude", language)}: {form.locationLng.toFixed(6)}
                          </Typography>
                        </Box>
                      )}
                    </Stack>
                  ...
                  <Box>
                    ...
                    <TextField
                      ...
                      multiline
                      rows={4}
                      placeholder={t("describeYourServices", language)}
                      sx={{
                        "& .MuiOutlinedInput-root": {
                          borderRadius: 2,
                          backgroundColor: "#fff",
                        ...
                        <label htmlFor="medicalLicense" style={{ width: "100%" }}>
                          <Button
                            ...
                            sx={{
                              ...
                              transition: "all 0.3s",
                              "&:hover": {
                                backgroundColor: "rgba(10, 93, 61, 0.1)",
                                borderColor: "#556B2F",
                              },
                            }}
                            startIcon={<CloudUploadIcon />}
                          >
                            {fileNames.medicalLicense ? `${fileNames.medicalLicense}` : t("pharmacistLicense", language)}
                          </Button>
                        </label>
                        <Typography variant="caption" sx={{ color: "#666", mt: 0.5, display: "block" }}>
                          {t("licenseToPracticePharmacy", language)}
                        </Typography>
                      </Box>
                      ...
                      <Box>
                        ...
                        <label htmlFor="universityDegree" style={{ width: "100%" }}>
                          <Button
                            ...
                            startIcon={<CloudUploadIcon />}
                          >
                            {fileNames.universityDegree ? `${fileNames.universityDegree}` : t("universityDegree", language)}
                          </Button>
                        </label>
                        <Typography variant="caption" sx={{ color: "#666", mt: 0.5, display: "block" }}>
                          {t("universityDegreeCertificate", language)}
                        </Typography>
                      </Box>
                      ...
                      <Box>
                        ...
                        <label htmlFor="idOrPassportCopy" style={{ width: "100%" }}>
                          <Button
                            ...
                            startIcon={<CloudUploadIcon />}
                          >
                            {fileNames.idOrPassportCopy ? `${fileNames.idOrPassportCopy}` : t("idPassportCopy", language)}
                          </Button>
                        </label>
                        <Typography variant="caption" sx={{ color: "#666", mt: 0.5, display: "block" }}>
                          {t("copyOfNationalIdOrPassport", language)}
                        </Typography>
                      </Box>
                      ...
                      <Box>
                        ...
                        <label htmlFor="clinicRegistration" style={{ width: "100%" }}>
                          <Button
                            ...
                            startIcon={<CloudUploadIcon />}
                          >
                            {fileNames.clinicRegistration ? `${fileNames.clinicRegistration}` : `${t("pharmacyRegistration", language)} (${t("optional", language)})`}
                          </Button>
                        </label>
                        <Typography variant="caption" sx={{ color: "#666", mt: 0.5, display: "block" }}>
                          {t("officialPharmacyRegistration", language)}
                        </Typography>
                      </Box>
                    </Stack>
                  </Box>
                  ...
                  <Button
                    ...
                    sx={{
                      ...
                      "&:hover": {
                        background: "linear-gradient(90deg, #7B8B5E, #556B2F)",
...
export default AddSearchProfilePharmacist;
