// src/Component/Manager/AdminFunctions/ManagerEmergencyRequests.jsx
// Manager wrapper for Emergency Requests with Manager Sidebar and Header
...
const ManagerEmergencyRequests = () => {
  ...
  // Status tabs configuration - supports both legacy and new status values
  const statusTabs = [
    {
      label: t("pending", language),
      statuses: ["PENDING_MEDICAL", "PENDING"],
      icon: <PendingIcon />,
      color: "#ff9800",
    },
    {
      label: t("approved", language),
      statuses: ["APPROVED_BY_MEDICAL", "APPROVED"],
      icon: <CheckCircleIcon />,
      color: "#4caf50",
    },
    {
      label: t("rejected", language),
      statuses: ["REJECTED_BY_MEDICAL", "REJECTED"],
      icon: <CancelIcon />,
      color: "#f44336",
    },
    {
  ...
  useEffect(() => {
    ...
    // Filter by status (supports multiple status values per tab)
    if (selectedTab < statusTabs.length - 1) {
      const selectedStatuses = statusTabs[selectedTab].statuses;
      filtered = filtered.filter((req) => selectedStatuses.includes(req.status));
    }
  ...
  const handleApprove = async (id) => {
    try {
      ...
      // api.get/patch returns response.data directly
      setRequests((prev) =>
...
export default ManagerEmergencyRequests;
