// src/Component/Sidebar/LabSidebar.jsx
...
const LabSidebar = memo(function LabSidebar({ activeView, setActiveView }) {
  ...
  // Restore last scroll
  useEffect(() => {
    const savedScroll = localStorage.getItem("labSidebarScroll");
  ...
  const handleScroll = () => {
    if (sidebarRef.current) {
      localStorage.setItem("labSidebarScroll", sidebarRef.current.scrollTop);
    }
  };
  ...
  const sidebarContent = (isDrawer = false) => (
    ...
    >
      ...
      >
        ...
        {/* Lab Requests */}
        <Tooltip title={t("labRequests", language)} placement={isRTL ? "left" : "right"} disableHoverListener={open || isDrawer}>
          <ListItemButton
          ...
          >
            ...
            {(open || isDrawer) && <ListItemText primary={t("labRequests", language)} />}
          </ListItemButton>
        </Tooltip>
        ...
        <Tooltip title={t("profiles", language)} placement={isRTL ? "left" : "right"} disableHoverListener={open || isDrawer}>
          <ListItemButton
            onClick={() => handleNavClick("labProfiles")}
            selected={activeView === "labProfiles"}
            sx={{ "&.Mui-selected": { backgroundColor: "rgba(255,255,255,0.2)" } }}
          >
            <ListItemIcon>
              <BiotechIcon sx={{ color: "#FF9800", fontSize: { xs: 26, sm: 22 } }} />
            </ListItemIcon>
    ...
    <>
      ...
      <LogoutDialog
        ...
        onConfirm={() => {
          ...
          localStorage.removeItem("labSidebarScroll");
