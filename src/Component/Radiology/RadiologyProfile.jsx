...
const RadiologyProfile = () => {
  ...
  const handleOpenFile = async (filename, documentName) => {
    ...
    try {
      ...
      setSnackbar({ open: true, message: `❌ Failed to open ${documentName}`, severity: "error" });
    }
  };

  // 🔹 Fetch Radiology Profiles
  const fetchProfiles = useCallback(async () => {
    try {
      ...
      setSnackbar({
        open: true,
        message: t("noRadiologyProfilesFound", language),
        severity: "warning",
      });
    } finally {
      setLoading(false);
  ...
  const handleDelete = async (id) => {
    if (!window.confirm(t("confirmDeleteRadiologyProfile", language)))
      return;
    try {
      await api.delete(API_ENDPOINTS.SEARCH_PROFILES.BY_ID(id));
      setSnackbar({
        open: true,
        message: t("radiologyProfileDeletedSuccessfully", language),
        severity: "success",
      });
      fetchProfiles();
    } catch {
      setSnackbar({
        open: true,
        message: t("failedToDeleteRadiologyProfile", language),
        severity: "error",
      });
    }
  };
  ...
  const handleEditSubmit = async () => {
    try {
      ...
      setSnackbar({
        open: true,
        message: t("radiologyProfileUpdatedSuccessfully", language),
        severity: "success",
      });
      ...
      setSnackbar({
        open: true,
        message: t("failedToUpdateRadiologyProfile", language) + ": " + (err.response?.data?.message || err.message),
        severity: "error",
      });
    }
  };
  ...
  return (
          ...
          <Stack direction="row" alignItems="center" spacing={2} mb={2}>
            ...
            <Box>
              <Typography variant="h4" fontWeight="700" sx={{ mb: 0.5 }}>
                {t("radiologyCenterProfiles", language)}
              </Typography>
              <Typography variant="body1" sx={{ opacity: 0.9 }}>
                {t("manageYourRadiologyFacilities", language)}
              </Typography>
            </Box>
          </Stack>
          ...
          {filteredProfiles.map((p) => {
            ...
            return (
              ...
              >
                ...
                {(p.medicalLicense || p.medicalLicenseFileName || p.universityDegree || p.universityDegreeFileName || p.idOrPassportCopy || p.idOrPassportCopyFileName || p.clinicRegistration || p.clinicRegistrationFileName) && (
                  <Box sx={{ mt: 2 }}>
                    <Typography variant="subtitle2" sx={{ fontWeight: "700", color: "#1e293b", mb: 1, fontSize: "0.85rem" }}>
                    ...
                    <Stack spacing={0.8}>
                      ...
                      >
                        <Box>
                          <Typography variant="caption" sx={{ fontWeight: "600", color: "#4A5D4A", fontSize: "0.75rem" }}>
                            📜 Radiologist License - رخصة مزاولة فني الاشعة
                          </Typography>
                        ...
                        {(p.medicalLicense || p.medicalLicenseFileName) && (
                          <Button
                            size="small"
                            variant="contained"
                            onClick={() => handleOpenFile(p.medicalLicense || p.medicalLicenseFileName, " Radiologist License - رخصة مزاولة فني الاشعة ")}
                      ...
                      {/* Radiology Registration (Optional) */}
                        ...
                        >
                          <Box>
                            <Typography variant="caption" sx={{ fontWeight: "600", color: "#E65100", fontSize: "0.75rem" }}>
                              🏢 Radiology Registration - تسجيل الأشعات
                            </Typography>
                            <Typography variant="caption" sx={{ display: "block", color: "#5d6b5d", fontSize: "0.7rem" }}>
                              {t("uploaded", language)}
                            </Typography>
                          ...
                          <Button
                            size="small"
                            variant="contained"
                            onClick={() => handleOpenFile(p.clinicRegistration || p.clinicRegistrationFileName, "Radiology Registration - تسجيل الأشعات")}
          ...
          {filteredProfiles.length === 0 && (
            <Box sx={{ gridColumn: "1 / -1", textAlign: "center", py: 8 }}>
              <Typography variant="h6" color="text.secondary">
                {t("noProfilesMatchYourSearch", language)}
              </Typography>
            </Box>
          )}
        </Box>
    ...
    <Dialog
      ...
      fullWidth
    >
      <DialogTitle sx={{ background: "linear-gradient(135deg, #556B2F 0%, #7B8B5E 100%)", color: "white", fontWeight: 700 }}>
        {t("editRadiologyProfile", language)}
      </DialogTitle>
        ...
        {selectedProfile && (
          <Stack spacing={2.5}>
            ...
            <Typography variant="caption" sx={{ color: "#666", fontSize: "0.85rem", fontStyle: "italic" }}>
              {t("editFieldsInstructions", language)}
            </Typography>
            ...
            <TextField
              ...
              fullWidth
              variant="outlined"
              placeholder={t("enterRadiologyCenterName", language)}
            />
            ...
            <TextField
              ...
              fullWidth
              variant="outlined"
              placeholder={t("enterContactInformation", language)}
            />
            ...
            <Box sx={{ p: 1.5, border: "1px solid #E8EDE0", borderRadius: 2, bgcolor: "#FAF8F5" }}>
              <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", mb: 1 }}>
                ...
                {selectedProfile.medicalLicense && typeof selectedProfile.medicalLicense === 'string' && (
                  <Chip label="✅ Already uploaded" size="small" color="success" variant="outlined" />
                )}
                {selectedProfile.medicalLicense instanceof File && (
                  <Chip label="📦 New file selected" size="small" color="primary" variant="outlined" />
                )}
              </Box>
              <Button
                variant="outlined"
            ...
            {/* Radiology Registration Upload */}
            <Box sx={{ p: 1.5, border: "1px solid #E8EDE0", borderRadius: 2, bgcolor: "#FAF8F5" }}>
              <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", mb: 1 }}>
                <Typography variant="caption" sx={{ fontWeight: "600", color: "#4A5D4A", fontSize: "0.8rem" }}>
                  🏢 Radiology Registration - تسجيل الأشعات (Optional)
                </Typography>
