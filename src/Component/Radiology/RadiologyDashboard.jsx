// src/Component/Radiology/RadiologyDashboard.jsx
...
const RadiologyDashboard = () => {
  ...
  const [activeView, setActiveView] = useState(
    localStorage.getItem("radiologyActiveView") || "dashboard"
  );
  useEffect(() => {
    localStorage.setItem("radiologyActiveView", activeView);
  }, [activeView]);

  // ✅ User Info
  const [userInfo, setUserInfo] = useState(() => {
    return safeJsonParse(localStorage.getItem("radiologyUserInfo"), {
  ...
  const [profileImage, setProfileImage] = useState(
    localStorage.getItem("radiologyProfileImage") || null
  );

  // ✅ Stats
  const [stats, setStats] = useState(() => {
    return safeJsonParse(localStorage.getItem("radiologyStats"), {
      pending: 0,
      completed: 0,
      total: 0,
    });
  ...
  const [requests, setRequests] = useState(() => {
    return safeJsonParse(localStorage.getItem("radiologyRequests"), []);
  });

  // ✅ Notifications
  const [unreadCount, setUnreadCount] = useState(
    Number(localStorage.getItem("radiologyUnreadCount")) || 0
  );
  ...
  const [currentRadiologyClaimData, setCurrentRadiologyClaimData] = useState(null);
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      if (userData) {
        ...
        localStorage.setItem("radiologyUserInfo", JSON.stringify(userData));
        ...
        if (imgPath) {
          ...
          localStorage.setItem("radiologyProfileImage", withTimestamp);
        } else {
          setProfileImage(null);
        }
      }

      // Requests - جلب الطلبات المعلقة والمكتملة
      let uniqueRequests = [];
      try {
        // api.get() returns data directly
        const pendingData = await api.get("/api/radiology/pending");
        const pendingList = Array.isArray(pendingData) ? pendingData : [];

        const myData = await api.get("/api/radiology/my-requests");
        const myList = Array.isArray(myData) ? myData : myData?.content || myData?.results || [];

        // دمج الطلبات المعلقة والمكتملة
        ...
        localStorage.setItem("radiologyRequests", JSON.stringify(uniqueRequests));

        // Stats - Calculate from actual requests data
        // Pending: All pending requests (available for this radiologist)
  ...
  // ✅ Submit Radiology Claim with Document
  const handleRadiologySubmitClaim = async (document, claimDataParam) => {
   
    // ✅ تحديد claimData للاستخدام - الأولوية: claimDataParam > currentRadiologyClaimData > window.radiologyClaimData
    ...
    // 2. استخدام currentRadiologyClaimData (state) إذا كان موجوداً
    else if (currentRadiologyClaimData && typeof currentRadiologyClaimData === 'object' && currentRadiologyClaimData !== null && currentRadiologyClaimData.clientId) {
     
      claimDataToUse = currentRadiologyClaimData;
    }
    // 3. استخدام window.radiologyClaimData كـ fallback
    else if (typeof window !== 'undefined' && window.radiologyClaimData && typeof window.radiologyClaimData === 'object' && window.radiologyClaimData.clientId) {
    ...
    try {
      ...
      // ✅ تنظيف currentRadiologyClaimData بعد النجاح
      setCurrentRadiologyClaimData(null);

      if (typeof window !== 'undefined') {
        if (window.radiologyClaimData) {
  ...
  return (
    ...
    >
      <RadiologySidebar activeView={activeView} setActiveView={setActiveView} />
            ...
            >
              <Box sx={{ display: "flex", alignItems: "center", gap: 3, position: "relative", zIndex: 1 }}>
                ...
                <Box sx={{ flex: 1 }}>
                  ...
                  >
                    {t("welcome", language)}, {userInfo?.fullName || t("radiologist", language)}!
                  </Typography>
                  <Typography
                    variant="body1"
                    sx={{
            ...
            <Grid container spacing={3} sx={{ mb: 5 }} justifyContent="center">
              ...
              <Grid item xs={12} sm={6} md={4}>
                ...
                >
                  ...
                  >
                    {t("pending", language)} {t("radiologyRequests", language)}
                  </Typography>
                  <Typography 
                    variant="h2" 
                    fontWeight="bold"
              ...
              <Grid item xs={12} sm={6} md={4}>
                ...
                >
                  ...
                  >
                    {t("completed", language)} {t("radiologyRequests", language)}
                  </Typography>
                  <Typography 
                    variant="h2" 
                    fontWeight="bold"
              ...
              <Grid item xs={12} sm={6} md={4}>
                ...
                >
                  ...
                  >
                    {t("total", language)} {t("radiologyRequests", language)}
                  </Typography>
                  <Typography 
                    variant="h2" 
                    fontWeight="bold"
                    ...
                    <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
                      ...
                      <Box>
                        <Typography variant="subtitle1" fontWeight="bold" sx={{ color: "#2E3B2D", mb: 0.5 }}>
                          {t("view", language)} {t("radiologyRequests", language)}
                        </Typography>
