// src/Component/Sidebar/RadiologySidebar.jsx
...
const RadiologySidebar = memo(function RadiologySidebar({ activeView, setActiveView }) {
  ...
  // Restore last scroll
  useEffect(() => {
    const savedScroll = localStorage.getItem("radiologySidebarScroll");
  ...
  const handleScroll = () => {
    if (sidebarRef.current) {
      localStorage.setItem("radiologySidebarScroll", sidebarRef.current.scrollTop);
    }
  };
  ...
  const sidebarContent = (isDrawer = false) => (
    ...
    >
      ...
      >
        ...
        {/* Radiology Requests */}
        <Tooltip title={t("radiologyRequests", language)} placement={isRTL ? "left" : "right"} disableHoverListener={open || isDrawer}>
          <ListItemButton
            onClick={() => handleNavClick("requests")}
            selected={activeView === "requests"}
            sx={{ "&.Mui-selected": { backgroundColor: "rgba(255,255,255,0.2)" } }}
          >
            <ListItemIcon>
              <LocalHospitalIcon sx={{ color: "#00BFFF", fontSize: { xs: 26, sm: 22 } }} />
            </ListItemIcon>
            {(open || isDrawer) && <ListItemText primary={t("radiologyRequests", language)} />}
          </ListItemButton>
        </Tooltip>
    ...
    <>
      ...
      <LogoutDialog
        ...
        onConfirm={() => {
          ...
          localStorage.removeItem("radiologySidebarScroll");
          localStorage.removeItem("radiologyUserInfo");
          localStorage.removeItem("radiologyStats");
          localStorage.removeItem("radiologyRequests");
          localStorage.removeItem("radiologyUnreadCount");
          localStorage.removeItem("radiologyProfileImage");
