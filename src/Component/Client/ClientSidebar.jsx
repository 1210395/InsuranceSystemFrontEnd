// src/Component/Sidebar/ClientSidebar.jsx
...
const ClientSidebar = memo(function ClientSidebar({ activeView, setActiveView }) {
  ...
  // Restore last scroll
  useEffect(() => {
    const savedScroll = localStorage.getItem("clientSidebarScroll");
  ...
  const handleScroll = () => {
    if (sidebarRef.current) {
      localStorage.setItem("clientSidebarScroll", sidebarRef.current.scrollTop);
    }
  };
    ...
    >
      ...
      >
        ...
        <Tooltip title={t("myClaims", language)} placement={isRTL ? "left" : "right"} disableHoverListener={open || isDrawer}>
          <ListItemButton
            selected={activeView === "claims"}
            onClick={() => handleNavClick("claims")}
            sx={{ "&.Mui-selected": { backgroundColor: "rgba(255,255,255,0.2)" } }}
          >
            <ListItemIcon>
              <AssignmentIcon sx={{ color: "#BA68C8", fontSize: { xs: 26, sm: 22 } }} />
            </ListItemIcon>
        ...
        <Tooltip title={t("addClaim", language)} placement={isRTL ? "left" : "right"} disableHoverListener={open || isDrawer}>
          <ListItemButton
            selected={activeView === "add-claims"}
            onClick={() => handleNavClick("add-claims")}
            sx={{ "&.Mui-selected": { backgroundColor: "rgba(255,255,255,0.2)" } }}
          >
            <ListItemIcon>
              <AssignmentIcon sx={{ color: "#FFB74D", fontSize: { xs: 26, sm: 22 } }} />
            </ListItemIcon>
        ...
        <Tooltip
          title={t("myPrescriptions", language)}
        ...
        >
          ...
          >
            ...
            {(open || isDrawer) && <ListItemText primary={t("myPrescriptions", language)} />}
          </ListItemButton>
        </Tooltip>

        <Tooltip
          title={t("myLabRequests", language)}
          placement={isRTL ? "left" : "right"}
          disableHoverListener={open || isDrawer}
        >
          <ListItemButton
            selected={activeView === "lab"}
            onClick={() => handleNavClick("lab")}
            sx={{ "&.Mui-selected": { backgroundColor: "rgba(255,255,255,0.2)" } }}
          >
            <ListItemIcon>
              <ScienceIcon sx={{ color: "#00BFFF", fontSize: { xs: 26, sm: 22 } }} />
            </ListItemIcon>
            {(open || isDrawer) && <ListItemText primary={t("myLabRequests", language)} />}
          </ListItemButton>
        </Tooltip>

        <Tooltip
          title={t("myRadiologyRequests", language)}
          placement={isRTL ? "left" : "right"}
          disableHoverListener={open || isDrawer}
        >
          <ListItemButton
            selected={activeView === "radiology"}
            onClick={() => handleNavClick("radiology")}
            sx={{ "&.Mui-selected": { backgroundColor: "rgba(255,255,255,0.2)" } }}
          >
            <ListItemIcon>
    ...
    <>
      ...
      <Box
        sx={{
          ...
          transition: "width 0.3s ease",
          zIndex: 1200,
          boxShadow: isRTL ? "-4px 0 15px rgba(0,0,0,0.3)" : "4px 0 15px rgba(0,0,0,0.3)",
        }}
      >
      ...
      <LogoutDialog
        ...
        onConfirm={() => {
          ...
          localStorage.removeItem("clientSidebarScroll");
